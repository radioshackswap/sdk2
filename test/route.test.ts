import { Token, WETH, ChainId, Pair, TokenAmount, Route, CETH } from '../src'
...
describe('Route', () => {
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], CETH_AVALANCHE, token0)
    expect(route.pools).toEqual([pair_0_weth])
    expect(route.input).toEqual(CETH_AVALANCHE)
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, CETH_AVALANCHE)
    expect(route.pools).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(CETH_AVALANCHE)
  })
})
