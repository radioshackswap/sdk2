...
describe('miscellaneous', () => {
  it('getLiquidityMinted:0', async () => {
    const tokenA = new Token(ChainId.AVALANCHE, '0x0000000000000000000000000000000000000001', 18)
    const tokenB = new Token(ChainId.AVALANCHE, '0x0000000000000000000000000000000000000002', 18)
    const pair = new Pair(new TokenAmount(tokenA, '0'), new TokenAmount(tokenB, '0'))

    expect(() => {
      pair.getLiquidityMinted(new TokenAmount(pair.liquidityToken, '0'), [
    ...
    const liquidity = pair.getLiquidityMinted(new TokenAmount(pair.liquidityToken, '0'), [
  ...
  it('getLiquidityMinted:!0', async () => {
    ...
    expect(
      pair
        .getLiquidityMinted(new TokenAmount(pair.liquidityToken, '10000'), [
  ...
  it('getLiquidityValue:!feeOn', async () => {
    ...
    {
      const [liquidityA] = pair.getLiquidityValues(
        new TokenAmount(pair.liquidityToken, '1000'),
        new TokenAmount(pair.liquidityToken, '1000')
      )
      expect(liquidityA.token.equals(tokenA)).toBe(true)
      expect(liquidityA.raw.toString()).toBe('1000')
    }
    ...
    // 500
    {
      ...
      expect(liquidityA.token.equals(tokenA)).toBe(true)
      expect(liquidityA.raw.toString()).toBe('500')
    }

    // tokenB
    {
      const [, liquidityB] = pair.getLiquidityValues(
        new TokenAmount(pair.liquidityToken, '1000'),
        new TokenAmount(pair.liquidityToken, '1000')
      )
      expect(liquidityB.token.equals(tokenB)).toBe(true)
      expect(liquidityB.raw.toString()).toBe('1000')
    }
  })
  ...
  it('getLiquidityValue:feeOn', async () => {
    ...
    expect(liquidityA.token.equals(tokenA)).toBe(true)
    expect(liquidityA.raw.toString()).toBe('917') // ceiling(1000 - (500 * (1 / 6)))
  })
