...
describe('Trade', () => {
  const CETH_AVALANCHE = CETH[ChainId.AVALANCHE]
  ...
  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], CETH[43114], token0),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(CETH_AVALANCHE)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with ETHER as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], CETH_AVALANCHE, token0),
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, CETH_AVALANCHE),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(CETH_AVALANCHE)
  })
  ...
  describe('#bestTradeExactIn', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactIn([], new TokenAmount(token0, JSBI.BigInt(100)), token2)).toThrow('Invariant failed: POOLS')
    })
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(CETH_AVALANCHE)
      expect(result[0].route.path).toEqual([WETH[ChainId.AVALANCHE], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(CETH_AVALANCHE)
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].outputAmount.currency).toEqual(CETH_AVALANCHE)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH[ChainId.AVALANCHE]])
      expect(result[1].outputAmount.currency).toEqual(CETH_AVALANCHE)
    })
  })
  ...
  describe('#bestTradeExactOut', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactOut([], token0, new TokenAmount(token2, JSBI.BigInt(100)))).toThrow('Invariant failed: POOLS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
