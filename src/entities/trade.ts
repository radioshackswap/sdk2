...
function wrappedAmount(currencyAmount: CurrencyAmount, chainId: ChainId = ChainId.AVALANCHE): TokenAmount {
  ...
  if (currencyAmount.currency === CETH[chainId]) return new TokenAmount(WETH[chainId], currencyAmount.raw)
  invariant(false, 'CURRENCY')
}

function wrappedCurrency(currency: Currency, chainId: ChainId = ChainId.AVALANCHE): Token {
  if (currency instanceof Token) return currency
  if (currency === CETH[chainId]) return WETH[chainId]
  invariant(false, 'CURRENCY')
}
  ...
  public constructor(
    ...
    this.inputAmount =
      tradeType === TradeType.EXACT_INPUT
        ? amount
        : route.input === CETH[chainId]
        ? CurrencyAmount.ether(amounts[0].raw, chainId)
        : amounts[0]
    this.outputAmount =
      tradeType === TradeType.EXACT_OUTPUT
        ? amount
        : route.output === CETH[chainId]
